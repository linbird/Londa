...
#### 实现原理
内核维护数据结构eventpoll用来管理所要监视的fd，该数据结构中核心在于一颗红黑树（每个节点为epitem结构，用来快速的查找和修改要监视的fd）和一个列表（收集已经发生事件的epitem）。[具体分析过程1](https://blog.csdn.net/baiye_xing/article/details/76352935) [具体分析过程2](https://blog.csdn.net/dog250/article/details/80837278)
...
2. 为epoll句柄添加epitem；注册睡眠entry的回调
   ...
   */
   ...
   SYSCALL_DEFINE4(epoll_ctl, int, epfd, int, op, int, fd, struct epoll_event __user *, event) {  
       ...
       copy_from_user(&epds, event, sizeof(struct epoll_event));//从用户空间将epoll_event结构copy到内核空间.
   	...
   	struct eventpoll *ep = file->private_data; // 取得内部结构eventpoll 
