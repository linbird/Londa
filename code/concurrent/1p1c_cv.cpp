...
void consumer(){
    ...
    while(!end_flag.load()){
        ...
        //        auto status = cv.wait_for(locker, 10ms, []{return ((data_pool.size() >= 100) || end_flag.load());});
        cv.wait(locker, []{return ((data_pool.size() >= 100) || end_flag.load());});
        if(end_flag.load()){
            fmt::print(fmt::bg(fmt::color::green) | fmt::fg(fmt::color::blue), fmt::format("{}：消费者({}) 收到结束请求\n", current_time(), std::this_thread::get_id()));
            fmt::print(fmt::fg(fmt::color::green) | fmt::fg(fmt::color::blue), fmt::format("{}：消费者({}) 数据池中剩余{}数据, 将一次读取完\n", current_time(), std::this_thread::get_id(), data_pool.size()));
        ...
        auto start = data_pool.begin(), end = std::next(data_pool.begin(), batch_size);
        copy(std::make_move_iterator(start), std::make_move_iterator(end), std::ostream_iterator<float>(cout, " "));
        cout << endl;
        data_pool.erase(start, end);
        ////////////////////\\\\\\\\\\\\\\\\\\\\\\\\//
...
int main(){
    ...
    std::array<thread, 2> threads{thread(producer, 50ms), thread(consumer)};
