    ...
    * [零拷贝](#零拷贝)
        ...
        * [`sendfile()`](#sendfile)
            * [函数原型](#函数原型-1)
                * [参数](#参数)
                * [返回值](#返回值-1)
            * [支持DAM聚合拷贝`sendfile`](#支持dam聚合拷贝sendfile)
        * [`splice()`](#splice)
            * [函数原型](#函数原型-2)
                * [参数](#参数-1)
                * [返回值](#返回值-2)
        * [[`tee()`](# 减少拷贝：`Linux::tee()`)](#tee-减少拷贝linuxtee)
        * [`vmsplice()`](#vmsplice)
    * [内存分配](#内存分配)
        * [`brk()/sbrk()`](#brksbrk)
            * [内存释放](#内存释放)
...
* [进程通信](#进程通信)
    ...
    * [信号 signal](#信号-signal)
        * [信号类别与状态](#信号类别与状态)
            * [信号分类 !Linux预定义的部分信号](#信号分类-linux预定义的部分信号)
            * [信号状态](#信号状态)
        * [信号产生](#信号产生)
            * [信号发生API](#信号发生api)
                * [与本进程相关](#与本进程相关)
        ...
        * [System V实现](#system-v实现)
            ...
            * [操作](#操作)
                ...
                * [使用](#使用)
                    ...
                    * [读写](#读写)
                    * [解除映射](#解除映射)
        * [[Posix实现](# 内存映射：`mmap()`)](#posix实现-内存映射mmap)
        * [方案总结](#方案总结)
    * [Socket](#socket)
        * [属性](#属性)
    ...
    * [信号量](#信号量)
        ...
        * [读写信号量](#读写信号量)
            ...
            * [释放信号量](#释放信号量)
                ...
                * [写者释放信号量](#写者释放信号量)
            * [身份转换](#身份转换)
        * [`Semaphore`与`Mutex`](#semaphore与mutex)
        * [NOTE](#note)
    * [总结](#总结)
        * [通信方式](#通信方式)
...
|      状态       |                             含义                             |
| :-------------: | :----------------------------------------------------------: |
|  未决`pending`  |                    从产生到抵达之间的状态                    |
| 递达`delivery`  |     实际执行信号处理信号的动作（三种：忽略、默认、处理）     |
| **阻塞`block`** | 收到信号不立即处理，此时信号将**保持未决状态**，解除阻塞后才执行抵达动作 |
...
用户态信号处理函数**不能接受和处理`SIGSTOP、SIGKILL`**信号。`SIGKILL`是为 `root` 提供了一种使进程**强制终止**方法，此时将会有操作系统直接回收该进程占用的资源，对于一些保存状态的应用就可能会导致异常。`SIGSTOP`用于 `Shell` 的任务管理，不能被用户屏蔽（[参考来源](https://gohalo.me/post/kernel-signal-introduce.html)）。一般**信号处理函数都比较简单**，这样有利于**降低竞争的风险**，在信号处理函数内部，**并非所有的系统调用以及库函数均可予以安全调用**，[只有可重入函数和异步信号安全函数才能被信号处理函数安全调用](https://www.cnblogs.com/jingyg/p/5183039.html)。针对信号处理器函数有两种常见设计：

**一：**信号处理器函数**设置全局性标志变量并退出**。主程序对此标志进行周期性检查，一旦被置位随即采取相应动作（当主程序若因监控一个或多个文件描述符的 I/O 状态而无法进行这种周期性检查时，则可令信号处理器函数向一专用管道写入一个字节的数据，同时将该管道的读取端置于主程序所监控的文件描述符范围之内）。

**二：**信号处理器函数**执行某种类型的清理动作**，接着终止进程或者使用非本地跳转将栈解开并将控制返回到主程序中的预定位置。
...
此时内核中的**缓冲区具有标识符**，其他的进程可以通过这个标识符访问同一个文件实现**同一主机上任意进程间的通信**。**标识符为一个可见于文件系统的文件名**，所以需要依靠一个具体的文件名，此时的命名文件仅仅起到提供`inode`供其他进程索引而已，**数据依然存在于内存缓冲页**面中。命名管道一旦建立，之后它的读、写以及关闭操作都与普通管道完全相同。
...
int mknod(const char *filename, mode_t mod, dev_t dev);//创建除目录外的普通或特殊文件(命名管道)
//@parameters
//	filename: 待创建的文件名
//	mod: 文件权限，只读只写会阻塞
...
#### [`FIFO`打开规则](https://blog.csdn.net/messiran10/article/details/52502870)

一旦设置了阻塞标志，调用`mkfifo`建立好`fifo`文件之后，那么管道的**两端读写必须分别打开，有任何一方未打开，则在调用open的时候就阻塞**。
...
![字节流套接字通信](img/Linux/TCP.png)

![数据报套接字通信](img/Linux/UDP.png)

### 属性

**域 domain**：指定套接字通信中所在的域；如AF_UNIX/AF_LOCAL（Unix Socket）、AF_INET（IPV4）、AF_INET6（IPV6）、AF_NETLINK（内核进程通信）……。

**类型 type**：代表通讯数据的语意，如**SOCK_STREAM**（有序，可靠，双工、有连接、支持带外传输的字节流）、**SOCK_DGRAM**（无连接、不可靠、固定最大长度）、SOCK_RAW（原始字节）……。

**协议 protocal**：代表通讯所用的协议类型，将`protocol`设为0系统就会自动选择适当的协议。

### UNIX Socket

也叫**UDS**（Unix Domain Socket），UDS通过本机内核传输原始数据，不需要经过网络协议栈的打包、拆包、校验等操作。对于本地套接字来说，**流式套接字**（SOCK_STREAM）是一个有顺序的、可靠的双向字节流，相当于在本地进程之间建立起一条数据通道；**数据报式套接字**（SOCK_DGRAM）相当于单纯的发送消息，在进程通信过程中，理论上可能会有信息丢失、复制或者不按先后次序到达的情况，但由于其在本地通信，不通过外界网络，这些情况出现的概率很小。 因此UNIX Socket常用于前后台进程通信，比如 `X Window`。

[Linux msghdr结构讲解](https://ivanzz1001.github.io/records/post/linux/2017/11/04/linux-msghdr#1-msghdr%E7%BB%93%E6%9E%84)[Linux网络编程 -- socketpair的使用](https://blog.csdn.net/y396397735/article/details/50684558)

#### 命名socket

UNIX流式套接字（**常用**）的通信双方都需要本地地址，其中服务端必须通过`struct sockaddr_un`变量明确指明地址、**客户端必须使用和服务端相同的命名方式**。socket进程通信的命名方式有两种：

**普通命名**：系统根据参数中的**绝对路径文件名**自动**创建同名的socket文件**，客户端建立链接的时候通过**读取此sock文件**来和服务端建立链接。

**抽象命名**：将`sockaddr_un::sun_path`的**第一字节置NULL（‘\0’）**即启用了该特性。此时系统**不需要创建socket文件、**客户端只需要知道名字就可以连接。

#### 匿名Socket

Linux实现了一个源自BSD的**`socketpair`**调用，该系统调用能创建一对已连接的UNIX族`socket`。在Linux中这一对`socket`中的任意一个`socket`都可以当做**可读写**的文件描述符使用。

```c
int socketpair(int domain, int type, int protocol, int sockfd[2]);//建立一对匿名的已连接的套接字
//@Parameters:
//	domain: 协议族，必须为AF_LOCAL(操作系统通常仅对UNIX域提供支持)
//	type: SOCK_STREAM（全双工流管道）、SOCK_DGRAM
//	protocal: 只能为0
//	sockfd: 用于保存创建的套接字对，每一个描述符都是一个可读写的套接口
//@Return value:
//	成功则返回0；失败返回-1并置errno
```
...
struct sockaddr_un {//UDS中的套接字地址（sockaddr_un(ix)，sockaddr_in(ternet)）
    sa_family_t sun_family;//套接子类型：AF_UNIX/AF_LOCAL
    char sun_path[UNIX_PATH_MAX];//套接字绝对文件路径名 
};
...
```c
int send(int s, const void *msg, int len, unsigned int falgs);//将数据由指定的socket传给对方主机
//@Parameters
//	s: 已建立好连线的socket
//	msg: 希望传输或存储数据的地址
//	flags: 一般设0，还可以取MSG_OOB、MSG_DONTROUTE、MSG_DONTWAIT、MSG_NOSIGNAL
//@Return value
//	成功则返回传输的字节数；失败返回-1并置errno
```
...
[iov_iter结构体](https://blog.csdn.net/C_JLMPC2009/article/details/102597790)

```c
struct msghdr {//消息头：可用于文件描述符传递，数字证书传递
    void *msg_name;//指向socket地址结构，表示发送或接收数据的地址
    int msg_namelen;//地址结构长度
    struct iov_iter msg_iter;//要发送或接受的实际数据,可能包含一个或者多个数据区
    void *msg_control;//指向struct cmsghdr结构的控制信息(辅助数据)
    __kernel_size_t msg_controllen;//控制信息缓冲区长度
    unsigned int msg_flags;//接收信息时的标志
    struct kiocb *msg_iocb;//异步请求控制块
};

struct iov_iter {//记录管理多个数据块,用于遍历iovec结构
...
struct iovec {//一个数据区的信息
    void __user *iov_base;//数据区地址
    __kernel_size_t iov_len;//数据区长度
};

struct cmsghdr {//控制信息，有专有函数访问接口
	__kernel_size_t	cmsg_len;//附属数据的字节计数，这包含结构头的尺寸。这个值是由CMSG_LEN()宏计算的。
	int cmsg_level;//原始的协议级别(例如，SOL_SOCKET)。
	int cmsg_type;//控制信息类型(例如，SCM_RIGHTS)。
};
```
...
```c
int sendmsg(int s, const struct msghdr *msg, unsigned int flags);//将数据由指定的socket传给对方主机
int recvmsg(int s, struct msghdr *msg, unsigned int flags);//接收远程主机经指定的socket传来的数据
//@Parameters
//	s: 已建立好连线的socket
//	msg: 希望传输或存储数据的地址
//	flags: 一般设0，还可以取MSG_OOB、MSG_DONTROUTE、MSG_DONTWAIT、MSG_NOSIGNAL
...
接受进程调用`recvmsg`接收到发送进程（**两进程不需要亲缘关系**）借助**`msghdr`辅助数据字段**使用`sendmsg`发送过来的文件描述符后，内核会在接受进程中**创建一个新的文件描述符**，该文件描述符和发送进程中的文件描述符指向文件打开表中的同一表项。两个文件描述符**共享同一个文件打开表项**从而实现了共用文件描述符的效果。[参考(EN)](https://copyconstruct.medium.com/file-descriptor-transfer-over-unix-domain-sockets-dcbbf5b3b6ec)

###### 传送用户凭证

也是通过**辅助数据的方式**将用户的认证数据发送给对应的客户端。需要将`msg_control->cmsg_type = SCM_CREDS`。其余的处理都差不多。
...
|      |    范围    |      效率      |        传递内容        |        权限        |
| :--: | :--------: | :------------: | :--------------------: | :----------------: |
| UNIX |    本地    | 高(不走协议栈) | 可传递`fd`、用户凭证等 | 命名`socket`需`RW` |
| BSD  | 本地或远程 |       低       |        普通数据        |                    |
...
**信号量其实是一个整型的计数器，主要用于实现进程间的互斥与同步，而不是用于缓存进程间通信的数据**。对信号量的访问（PV操作）都具有原子性。
...
//	val: 信号量初值
//@Return value
//	成功返回sem_t指针；失败返回SEM_FAILED((void*)-1)并设置errno。
...
在32bit Linux中内核简化了分段机制（Intel由于历史原因必须支持分段），将最高的1GB虚拟内存空间作为内核空间（内核对应的虚拟地址空间对应的物理内存会**常驻内存**，不会被OS换出到磁盘等设备，**所有的进程共享内核空间地址**），对内核空间的访问将受到硬件保护(0级才可访问)，低3GB虚拟内存空间作为用户空间（包含代码段、全局变量、BSS、函数栈、堆内存、映射区等）供用户进程使用（3级可访问）。
...
由于开启了分页机制，**内核需要访问全部物理地址空间**的话，必须先建立映射关系。Linux内部的地址的映射过程为**逻辑地址**–>**线性地址**–>**物理地址**。物理地址即地址总线中传输的数字信号，而线性地址和逻辑地址所表示的则是一种转换规则。在Linux中简化了分段机制使逻辑地址等于线性地址，所以只需要借助MMU实现逻辑地址到物理地址的转换，这部分工作由MMU完成，涉及到的寄存器主要有CR0、CR3。
...
![物理内存的延迟分配](img/Linux/heapMapped.png)
...
|   BSS    |       未初始化或初始化为0的全局变量和静态变量（全为0）       |   静态   |        编译时确定        |  用户  |
...
进程的多个段的连续地址空间构成多个独立的内存区域。Linux内核使用**`vm_area_struct`结构**（包含区域起始和终止地址、指向该区域支持的系统调用函数集合的**`vm_ops`指针**）来表示一个独立的虚拟内存区域。

堆、栈这样的段一般都对应一个单独的VMA（无规定，但X86默认这样布局），但`mmap`段例外，它可能拥有的多个`vm_area_struct`段，系统将这些段链接接起来方便进程访问（少时用链表、多时用红黑树）。

![Linux进程内存管理](img/Linux/vm_area_struct.png)

![mmap的多个vm_area_struct分布](img/Linux/memoryDescriptorAndMemoryAreas.png)

## [物理内存管理](https://blog.csdn.net/gatieme/category_6393814.html)


| 简称 |       全程        |    来源     |        内容         |            数据结构             |
| :--: | :---------------: | :---------: | :-----------------: | :-----------------------------: |
| PFN  | Page Frame number | 物理地址/4K |      页帧编号       | `typedef struct _MMPFN{} MMPFN` |
| PTE  | Page Table Entry  |             | 指向页帧编号+页属性 |  `task_struct::mm_struct::pgd`  |

**PFN数据库**：将每一个页帧的属性地址提取出来组成一数据结构，将连续的PFN组织在一起被称为页帧数据库。

### PTE表项解读

![PTE结构](img/Linux/x86PageTableEntry4KB.png)

| $P_{resent}$ | R/W  | U/S  | PWT  | PCD  | $A_{ccess}$ | $D_{rity}$ | PAT  |  G   | avail | PTE  |
| :----------: | :--: | :--: | :--: | :--: | :---------: | :--------: | :--: | :--: | :---: | :--: |

PTE按4KB对齐保存了指向这个页的起始物理地址。这将可访问的物理内存限制为[4GB](http://www.google.com/search?hl=en&q=2^20+*+2^12+bytes+in+GB)。其它PTE字段都是为物理地址扩展（PAE）准备的。
...
`struct page`描述和管理每一个`4KB`的物理内存**不关注这段内存中的数据变化**。系统中所有的页面都存储在各自节点下的各个区域下的`zone_mem_map[]`中，通过该数组可以找到系统中的每一页（空闲或非空闲）。而其中的空闲页面则可由下文提到的以伙伴关系组织的空闲页链表`free_area[MAX_ORDER]`来索引。

在实际的`struct page`中为了节省内存使用了大量的`union`结构，使得同一个元素在不同的场景下有不同的意义，以下的数据结构只是抽象描述。
