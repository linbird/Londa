...
两种锁的实现都是在`inode`上进行加锁，但是各自认为锁的[持有者不同](https://yxkemiya.github.io/2019/08/19/file-lock/)：BSD认为锁的持有者是系统级打开文件表、而POSIX认为是进程持有了锁。因此他们在面对同一种情况时所采取的[处理策略也就不同](https://www.cnblogs.com/charlesblc/p/6287631.html) 。

| 实现  |   API   |                锁类型                |    NFS     | exec后锁继承 |
| :---: | :-----: | :----------------------------------: | :--------: | :----------: |
|  BSD  | `flock` |    文件锁；共享锁/排它锁；建议锁     | 不原生支持 |     继承     |
| POSIX | `fcntl` | 文件锁/记录锁；排它锁；建议锁/强制锁 |    支持    |     继承     |

①：遍历`inode::i_flock`链表**发现存在PID相同的锁时**：`fcntl`允许同一个进程对同一个文件多次加同样一把锁，而解锁只需一次完成；`flock`则不支持这样操作（共享锁可以多次加锁而除外）。

②：关闭一个描述符时：`fcntl`会关闭**本进程设置**的**关于本描述符的一切文件锁**，而`flock`不同。

③：多线程共用锁：可以共享使用文件锁、但两种实现都要求线程单独使用各自`open`返回的文件描述符。

④：多进程共用锁：`fcntl`可以使用父进程的文件描述符但不能使用使用锁；`fcntl`可以使用父进程的锁但是必须使用自己`open`文件返回的文件描述符。[来源](https://www.codenong.com/cs105605976/)

⑤：关闭进程与释放锁：`fcntl`会释放**本进程能够引用到的所有文件锁**。`flock`只有在一个**文件描述符及其所有副本关闭时才会释放**对其建立的锁。[来源](https://www.jianshu.com/p/643cbd96a00c)
...
①、在单个进程下，如果在已有的锁上加新锁，则**新锁会替换旧锁**，也不管锁是何种类型。在多进程下遵循读写锁的一般规则。
...
③、在本地文件系统上对同一个文件使用上面两种机制的锁时互不影响 。在NFS上由于`flock` 底层实现仿造 `fcntl` 的字节范围锁，所以两者会产生交互。
