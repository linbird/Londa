...
* [内存模型与原子操作](#内存模型与原子操作)
    ...
    * [原子类型](#原子类型)
        * [标准原子类型](#标准原子类型)
        * [指针原子类型`std::atomic<T*>`](#指针原子类型stdatomict)
        * [自定义原子类型](#自定义原子类型)
    * [原子操作支持](#原子操作支持)
...
+ mutex实现了锁用来控制对共享数据的访问，提供了`lock`和`unlock`两个成员方法实现对数据加锁和解锁，处于两者之间的代码受到了`mutex`的互斥访问控制。
...
`notify_all()/ notify_one()`：将条件变量中由于调用`wait()`而等待的的所有或一个线程唤醒，被唤醒的线程会尝试获取互斥锁并再次判断条件是否满足。**`notify`不应该处于互斥锁的保护范围内**，因为这样会使被唤醒的进程等待唤醒它的进程释放锁而进入阻塞状态，所以在**条件变量唤醒之前应该`unlock()`**。
