...
![](/home/linbird/Londa/img/DateBase/集合.png)
...
![](/home/linbird/Londa/img/DateBase/有序集合.png)
...
有序集合可以由 [ziplist (压缩列表)](http://blog.laoyu.site/2019/redis/Redis数据结构——压缩列表/) 或 [skiplist (跳跃表)](https://blog.laoyu.site/2019/redis/Redi数据结构——跳跃表/) 编码。当有序集合较小**（元素个数小于128且每个元素长度小于64B）时使用`ziplist`**，否则采用`skiplist`。

#### `hash`哈希

在redis中，哈希类型是指Redis键值对中的值本身又是一个键值对结构，形如`value=[{field1，value1}，...{fieldN，valueN}]`。

![](/home/linbird/Londa/img/DateBase/Redis-Hash (1).png)
...
Hash支持ziplist和hashtable两种编码方式，当字典中键值对个数小于512且键和值大小都小于64B是使用ziplist编码方式。
