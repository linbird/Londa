...
不同的编译器一般都会有默认的对齐量，一般都为2的幂次。在C中可以通过预编译命令 `#pragma pack(n)`修改对齐量，C++中提供了**`alignas`**用于指定对齐量，可以应用于类/结构体/union/枚举的声明/定义、非位域的成员变量的定义、变量的定义（除了函数参数或异常捕获的参数），`alignas`会对对齐量做检查，**对齐量不能小于默认对齐**。对应的C++还提供了 `alignof operator`，操作数如果是引用就返回引用类型的对齐方式、如果是数组则返回元素类型的对齐方式。
...
对于 `new T initializer;`形式的调用，C++`new` 表达式会先调用 `operator new` 分配一块足以放下一个 `T` 类型的值的内存，然后再根据 `initializer` 对 `T` 类型的值进行**初始化**。C++ 的初始化是一个[专门的复杂主题](https://zh.cppreference.com/w/cpp/language/initialization)，不同的场景、不同的语法和不同的类型会导致完全不同的初始化方式和初始化结果，调用构造器只是初始化的其中一种方式，且只适用于 ` class type` 和 ` union type`。
...
来源于boost库中的一个模板基类（已进入标准库），该模板类提供了`shared_from_this()`
...
4. 底层为哈希表 ( `unordered_map,unordered_set,unordered_multimap,unordered_multiset`)在删除后除删除点之外均有效，插入后引用均合法，如果**插入导致了重哈希则迭代器失效**（少见）。
5. [`deque`](https://blog.csdn.net/ww32zz/article/details/48224941)在插入首尾失效，删除则仅操作点失效==需要补充==
...
2. 内存池：缓解小块内存分配带来的问题，基本的思想是一次向 `heap`申请一块很大的内存（内存池），如果申请小块内存的话就直接到内存池中去取。
...
1. 概述：一级空间配置器（大于128B的大块内存分配，封装了 `malloc`与 `free`）和二级空间配置器（不大于128B的小块在内存池分配）。

![STL两级空间配置器](./img/lang/2layer-allocater.png)
...
![迭代器类型](./img/lang/cpp_iterator_top.jpg)
...
+ 通过移动迭代器将容器中某区域的元素移动到了其他容器中，该区域内仍可能残留有之前存储的元素，但这些元素是不能再被使用的，否则极有可能使程序产生各种其他错误。|                                        迭代器适配器                                        |                                          创建函数                                          |      要求      |
  | :----------------------------------------------------------------------------------------: | :----------------------------------------------------------------------------------------: | :------------: |
...
int a[5];//数组名a代表整个数组的首地址，和&a[0]在数值上是等价的，对他们进行算术运算的步长都是数组首元素类型的大小
int b[2][3];//b代表整个数组的首地址，b[0]表示的是第一行的首地址，两者在数值上具有等价关系
auto c = &a;//对数组名进行取地址操作得到的是整个数组的地址，对其进行算术操作的步长是整个数组的大小
```

1. 数组名的内涵在于其指代实体是一种数据结构，这种数据结构就是数组；
2. 数组名的外延在于其可以转换为指向其指代实体的指针，而且是一个只读指针变量 `T* const`（由于编译器会阻止所有对只读变量的修改，所以数组名不能进行自增自减的操作），对其进行 `sizeof`操作时得到的是数组整体的空间的大小.
3. 当数组名作为函数的参数进行传递时，作为常量指针的数组名的值被赋值到一个指针变量
4. 指向数组的指针则是另外一种变量类型（在WIN32平台下，长度为4），仅仅意味着数组的存放地址！

数组名和&数组名虽然在数值上相同，但是在类型上不同——数组名是指向首个元素的指针，&数组名是指向整个数组的指针。在指针加减整数的时候，前者以一个元素为颗粒度，后者以整个数组为颗粒度。

#### 类成员指针

要使用（&类名::成员名）获取指向成员的指针这个成员必须是对外可见的：

| 指向      | 申明方式                             | 赋值                        | 调用                                       |
|:-------:|:--------------------------------:|:-------------------------:|:----------------------------------------:|
| 类的普通成员  | `typedef 返回值 (类名::*指针类型名)(参数列表)` | `指针类型名  指针名 = &类名::成员函数名` | `(类对象.*指针名)(参数列表)`或者 `(类指针->*指针名)(参数列表)` |
| 类普通数据成员 | `typedef 数据类型名 (类名::*指针类型名)`     | `指针类型名  指针名 = &类名::成员变量名` | `类对象.*指针名`或者 `类指针->*指针名`                 |

类的静态成员和普通成员的区别在于，他们是不依赖于具体对象的，所有实例化的对象都共享同一个静态成员，所以静态成员也没有this指针的概念，所以指向类的静态成员的指针就是普通的指针。
...
2. `regex_search()`:在目标字符串中搜索匹配正则表达式的部分查找结果包含了匹配的子串和子串在目标字符串中的起始和结束地址。
...
+ 缓冲刷新/同步
  ...
  2. 动作：输出缓冲区的刷新动作一般导致缓冲区内的内容被执行IO到其关联的文件描述符中，然后清空缓冲区。而对于输入缓冲区，不可能强制实现将输入缓冲区强制注入到关联的实体中，因为输入是不可预测掌控的，所以对输入缓冲区的刷新表现为清空缓冲区里的当前内容，等待输入进入输入缓冲区。| 缓冲类型 |                         特点                         |    典型代表    |
     | :------: | :---------------------------------------------------: | :------------: |
...
+ **NOTE：**
  1. `auto`仅仅是一个占位符，在编译期间会自动推导出变量的类型，C++中的变量依然有明确的类型。
  2. `auto`表达式推导出的类型和 `auto`被替换时的类型不同，被替换成的字符只是最终类型的一部分。

### [推导规则](https://www.cnblogs.com/QG-whz/p/4951177.html)与注意事项

+ 由于 `auto`用来做类型推导依赖于初始化表达式，所以 `auto`类型的变量必须在声明的时候进行初始化。且当一个 `auto`修饰多个变量时，变量推导出的类型不能有冲突。

+ 使用auto关键字做类型自动推导时，依次施加一下规则：
  
  1. 当 `auto`不带有其他符号，且其初始化表达式是一个引用时，`auto` 会抛弃引用，直接推导出原始类型。
  2. 当 `auto`不带有其他符号，且其初始化表达式带有cv限定时，`auto` 会抛弃cv限定，直接推导出原始类型。
  3. 当 `auto`带有&符号时，会保留初始化表达式中带有的cv限定
  4. 初始化表达式为数组时，auto关键字推导类型为指针
  5. 当 `auto`带有&符号，且初始化表达式为数组时，则推导类型为数组类型

+ 小心用于只支持移动语义的类型（因为某些推导出的类型可能触发复制操作）
...
1. 如果 `exp`是一个不被括号 `( )`包围的表达式，或者是一个类成员访问表达式，或者是一个**单独的**变量，那么 `decltype(exp)`的类型就和 `exp`一致。
...
+ `decltype`：保留 `cv`限定符
+ `auto`：如果表达式的类型是引用，保留cv限定符，否则丢弃限定符。
...
1. 原生指针：利用模板类的偏特化，为原生的指针和 `const`指针提供 `iterator_traits`。
...
在编译一个模板函数时，编译器需要根据实参expr确定模板函数的两部分（模板参数T和模板变量param的类型ParamType），由于ParamType可以是T和cv、*、&、&&的结合，所以T和ParamType通常是不同的，其中**T的类型不仅取决于实参类型，也与`ParamType`相关**。 **一个变量param的类型ParamType由三部分构成**，包含：

1. 最基础的无修饰类型，如int、string

2. 第一类修饰：不改变变量的底层存储类型，只改变变量的访问方式和权限，如const和volatile

3. 第二类修饰：改变了变量的底层存储类型，如\*、&、[]等 

```cpp
// 模板参数值param类型 ?= (模板参数T类型 == 模板形参xT类型) ?= 模板实参/表达式expr类型
// 注：参数类型（即param的类型）是模板形参与其他标识符的结合，不一定是T类型

template<typename T> // T:模板参数类型（即T的类型）
void f(ParamType  param); // ParamType：模板形参，T或者T与cv、*、&、&&结合
...
此时参数采用值传递的方式进行，即意味着函数在修改传入参数后不会影响到调用者，因此保留表达式的CV限定无意义，此时**实参中的第一类修饰（CV修饰）将会被忽略**。注意这里忽略的是实参的值类型的const属性，而不是实参存储的值的const属性。

|                   | ParamType = T                    |
| ----------------- | -------------------------------- |
| 实参为指针        | 保留实参cv限定，指针类型         |
| 实参为其他左/右值 | 不保留cv限定，值类型             |
| 实参为引用        | 不保留cv限定，值类型、非引用类型 |
| 实参为数组        | 指针类型                         |
| 实参为函数        | 函数指针类型                     |

###### 模板实参为指针

当实参为指针时，推导出的模板参数T和模板函数值类型将保留表达式中的cv限定符和指针类型，即**T和param都是指针类型，且保留cv限定符**。
...
f(p); // T和param都是int*
f(q); // T和param都是const int*，const属性被保留
f(ptr);  // T和param都是const char*, ptr的const属性被忽略，ptr指向的值的const属性被保留
```

###### 模板实参非指针（**衰减**）

当调用模板函数的参数表达式不是指针时，推断出的模板参数会出现**衰减(decaying)**（模板实参与推导出的模板参数类型不同：如数组和函数类型退化为普通指针、**丢失 `const`和 `volatile`属性、丢失引用类型**）。此时T和Param都是非引用类型，且cv限定符被丢弃。
...
f(expr); // T和param都是int
f(cx);// T和param都是int, const属性被忽略
f(rx);// T和param都是int, const属性被忽略
```
...
|                    | ParamType = T&         | ParamType = T*   | ParamType = T&&          |
| ------------------ | ---------------------- | ---------------- | ------------------------ |
| 实参为指针         | 保留cv，指针的左值引用 | 保留cv，指针类型 | 保留cv在，指针的左值引用 |
| 实参为左值         | 保留cv，左值引用       | 保留cv，指针类型 | 保留cv，左值引用         |
| 实参为左值引用     | 保留cv，左值引用       | 保留cv，指针类型 | 保留cv，左值引用         |
| 实参为右值（无cv） | ⚠️ERROR                 | 保留cv，指针类型 | 右值引用                 |
| 实参为右值引用     | 保留cv，左值引用       | 保留cv，指针类型 | 保留cv，右值引用         |
| 实参为数组         | 数组类型的左值引用     | 指针类型         | 数组类型的左值引用       |
| 实参为函数         | 函数指针的引用         | 函数指针         | 函数指针的左值引用       |

###### 模板形参为T&（引用传递）

和值传递相反，引用传递中对参数的修改需要影响到调用者，因此其推导T将**保留表达式中的 `cv`限定符**,如果**表达式本身和形参中含有任意个右值引用会涉及到引用折叠**，否则会被推导为左值引用类型。具体而言即**param一定是左值引用类型**（表达式是指针则param是指针的引用），**T一定是非引用类型**。

```cpp
// 引用传递（第二种模型）
template<typename T> void f(T& param); // 此为引用，但可能涉及到引用折叠

int i, &r, &&rr, *p;
const int& cr;
const int* const q;

f(i); // param是int& => T&是int& => T是int
f(r); // param是int& => T&是int& => T是int
f(rr); // param是int& => T&是int& => T是int
f(cr); // param是const int& => T&是const int& => T是const int
f(p); // param是int* & => T&是int* & => T是int*
f(q); // param是const int* const & => T&是const int* const & => T是const int* const
```

如果实参为数组，则param会被推导成为一个数组
```cpp
template<typename T> void f1(T param) {}   // param现在是pass-by-value
template<typename T> void f(T& param) {} 

const char name[] = "J. P. Briggs";// name的类型是const char[13]
const char * ptrToName = name;  // 数组被退化成指向数组的指针

f1(name);  // 给参数传递的是一个const char[13]类型
f1(ptrToName);  // 给参数传递的是一个const char *类型
...
与T&类似，**param一定是指针类型，cv限定符保留**，**T一定是非引用类型**。

```cpp
template<typename T> void f(T* param); // 此为地址传递，不会发生推断衰减
...
通用引用类型参数（即T&&）与右值引用参数的形式是一样的，但是万能引用接受左值。在这种情况下**param一定是引用类型**，至于是左值引用还是右值引用取决于实参：

实参是左值：**param是左值引用，cv限定符保留**。
...
//由于模板类型参数template <typename Elem> class Cont中的Elem没有被用到，因此可以省略
template <typename T, template <typename> class Cont = std::deque> class Stack {};
//使用时只需要指定类型T一次
Stack<int, std::vector> vStack;

//在实际编译过程中Cout = std::deque会由于模板参数不匹配会报错，这是因为deque需要两个模板参数
template <class _Tp, class _Allocator = allocator<_Tp> > class _LIBCPP_TEMPLATE_VIS deque;
//方案1:将模板参数补齐即可，对于没用到的模板参数也可以省略，因此可以定义
template <typename T, template <typename, typename> class Cout = std::deque> class Stack {};
//方案2:利用可变模板参数
template <typename T, tempalte <typename...> class Cout = std::deque> class Stack {};
```
...
所以把模板的声明和定义都放在一起构成 `hpp`文件（如STL），或者在模板的定义文件中加入所有可能的模板函数调用（傻雕写法），此外也可以利用 `export`关键字

#### `export`（C++11前）

为了访问其他编译单元（如另一代码文件）中的变量或对象，对普通类型（包括基本数据类、结构和类），可以利用关键字 `extern`来使用这些变量或对象时；但是对模板类型，则必须在定义这些模板类对象和模板函数时，使用标准C++新增加的关键字 `export`：在头文件中声明模板类和模板函，在代码文件中使用关键字 `export`来定义具体的模板类对象和模板函数，然后在其他用户代码文件中，包含声明头文件后就可以使用该这些对象和函数了。
...
把构造的任务委派给目标构造函数来完成类的构造，避免多个构造函数执行类似操作（**编译器原则上不允许在构造函数中调用任何构造函数**止），每个构造函数负责处理自己的不同情况，把最基本的构造工作委托给某个基础构造函数完成，实现分工协作。
...
4. 目标/被委托的构造函数可以是 `private`的构造函数

##### 禁止嵌套调用构造函数

C++不支持在构造函数中再次调用构造函数，当在构造函数内部调用构造函数时相当于创建了一个匿名的临时对象，被调用的构造函数完成了匿名临时对象的初始化，这一部分内容不会反映到当前对象的存储空间内，相当于做了无用功。**要想复用其他构造函数可以采用委托构造函数或者 `placement new`方法**。

#### 异常处理

如果在委派构造函数中使用 `try`的话，那么从目标构造函数中产生的异常，都可以在委派构造函数中被捕捉到。
...
2. `explicit`可以阻止构造函数的这种转换，避免一些难以发现的错误。

### 构造函数

构造函数的执行过程会分成**两个阶段**：初始化阶段（变量或者类在定义和分配空间后的第一次赋予初值即为初始化，按照申明和继承的顺序初始化）和赋值阶段（构造函数体内的部分）。

由于**`const` 成员**或**引用类型**的成员只有一次赋值的机会，所以他们必须在初始化列表处进行初始化。

#### [默认构造函数](https://blog.csdn.net/hankai1024/article/details/7947989)

##### 平凡默认构造函数（trivial constructor）

当用户并没有显示地定义默认构造函数时，**出于保证程序能够正确运行的需求**，编译器会自动生成一个无关紧要（trivial）的默认构造函数，**该函数既不会讲其成员变量置零，也不会做其他的任何事情**，没有实际意义。

##### 非平凡默认构造函数（non-trivial constructor）

该类构造函数是类实现所必须的，在必要的时候它们会由编译器自动生成。

### 自增/减运算符重载

后缀形式的自增/减重载接受一个无用的 `int` 形参来与前缀形式区分

### [`inline`函数](https://www.cnblogs.com/fnlingnzb-learner/p/6423917.html)

普通的 `inline`函数没有函数地址，在编译时进行类型检查将代码展开替换。在类中声明并定义的所有函数默认都是内联函数，但和 `rigister`关键字一样，`inline`对于编译器来说只是一个建议，并不强制要求将其做内敛展开，事实上，**编译器会拒绝将复杂的函数以及虚函数进行内联展开**，所以 `inline`和 `virtual`可以同时使用，此时编译器会忽略 `inline`而保留 `virtual`。

#### 注意事项

1. 内联函数的函数体内部不支持复杂的控制逻辑，如while、switch的结构会被编译器拒绝inline
2. 内联函数不能是递归的函数
3. 建议inline函数的定义放在头文件中(编译器必须随处可见内联函数的定义)
4. 关键字inline 必须与函数定义体放在一起才能使函数成为内联，仅将inline 放在函数声明前面不起任何作用。
5. 要当心构造函数和析构函数可能会隐藏一些行为，如“偷偷地”执行了基类或成员对象的构造函数和析构函数。所以**不要随便地将构造函数和析构函数的定义体放在类声明中**。
...
| **构造函数**   | 对象中的虚函数表指针和虚函数表在构造函数的初始化列表阶段（编译期）分配 |
...
如果一个类定义在另一个类的内部（无论在protect、private或者public中），这个内部类就叫做内部类，内部类是一个独立的不属于外部类的类，不能通过外部类的对象去调用内部类。**外部类对内部类没有任何优越的访问权限，但内部类是外部类的友元类**，内部类可以通过外部类的对象参数来访问外部类中的所有成员，直接访问外部类中的static、枚举成员（不需要外部类的对象/类名）。

```cpp
class Out{   
public : 
  void Fun () {
       nested object; // 这一行编译通过
       object.b_ = 1; // 这一行编译不通过！ int Out::nested::b_' is private within this context
  }
...
**NOTE**: C++不允许内部类型(内部类与内部typedef类型)的前置声明！

```cpp
class Container::Iterator;
void Fun(Container::Iterator iter) { }// 编译报错
class Container{
  class Iterator{ };
};
...
```cpp
// 奇技淫巧：内嵌类访问外部类数据成员（利用类内存布局中的地址偏移：offsetof宏）
// 嵌套类访问外围类中的成员变量
class GH_A{
  private:
...
// cstddef中定义的宏：其值是从指定类型对象开始到其指定子对象的字节数偏移
#define offsetof(type, member) /*implementation-defined*/
```

### 内部类与模板

在模板类中申明一个嵌套类之后，由于嵌套类可以直接使用外部模板类的模板类型，这样在需要使用内部类的时候将不需要通过模板参数实例化内部类。
...
template<typename Key> class SkipList {
  private:
    class SkipNode; //内部类申明
    SkipNode head_;
  public:
    SkipList(Key value) : head_(value) {}    
...
template<typename Key> Key SkipList<Key>::PutKey(typename SkipList<Key>::SkipNode node) {
  cout << node.key_ << endl;
  return node.key_;
}

// 内部类的实际定义
template<typename Key> class SkipList<Key>::SkipNode {
  public:
    Key key_;
    SkipNode(Key value) :key_(value) {}
};

int main() {
  SkipList<string> tmp("Hello World!");
  cout << tmp.PrintHead() << endl;
  return 0;
}
```
...
当**修饰类的非静态成员函数**保证该函数所对应的对象本身的 `const`性。在 `const`成员函数中，所有可能违背 `this`指针（此时等同于**`const Type * const this`**）类型特点的操作都会被阻止（赋值、对非 `const`成员函数的调用、对成员的非 `const`成员函数的调用，对非 `mutable`成员的修改）。但**对声明为 `mutable`的成员变量所做的任何操作都不会被阻止**。

**`const`修饰函数时还会参与到函数的重载中**，即通过 `const`对象、`const`指针或引用调用方法时，优先调用 `const`方法，**当没有一般成员函数时，非 `const`对象/引用/指针也会调用 `const`成员方法。**

**重载原因**（个人推测）：类成员函数在编译时会被自动加入 `this`指针参数，如前文所述，const成员函数传递过来的 `this`指针的类型是带有 `const`限定的，此时两种函数的函数签名也就是不同的。
...
Test test;
const char* py = &test.y;//error:undefined reference to `Test::y',需要在类外提供定义
```
...
- 用户定义的类类型转换：类中的构造函数，类型转换函数等。
  ...
  ![参数类型匹配规则](./img/lang/pair-order.jpg)
...
| 字符  | `const<w>````````char<8 | 16                                                                                     |
| 字符串 | `const<w>````````char<8 | 16                                                                                     |
...
将二元运算（`+`、`-`、`*`、` /`、` %`、` ^`、` &`、` |`、` =`、` <`、` >`、` <<`、` >>`、` +=`、` -=`、` *=`、` /=`、` %=`、` ^=`、` &=`、` |=`、` <<=`、` >>=`、` ==`、` !=`、` <=`、` >=`、` &&`、`||`、`,`、` .*`、` ->*`）与参数包结合，将参数包中的参数展开依次进行运算。Fold表达式一共有四种形式(二元折叠要求两个op必须相同，所有 `op`中只有 `&&`、`||`、 `,`支持pack为空，此时分别代表 `true、false、void()`)。
...
C++/C 程序设计风格的一个基本原则：声明与定义不可混为一谈，用户在阅读源代码声明时没有必要、也不应该知道函数是否需要内联、默认参数是什么。

##### 头文件

一般是在头文件中给出类的定义或全局函数的声明信息，而在代码文件中给出具体的（类成员函数或全局函数的）函数定义。然后在多个用户代码文件中包含该头文件后，就可以使用其中定义或声明的类和函数。

头文件中一般不包含变量、结构和类对象的定义，因为这样可能会导致重复定义的编译错误。解决办法是，在某个代码文件中进行定义，在其他用户代码文件中用 `extern`来引用它们，对于模板则使用 `export`关键字。
...
1. 宏的嵌套定义：**从最外层开始** （不同于函数调用最终先执行参数），每一层替换将替换执行的结果返回，进行下一层的替换。
2. 宏展开是**不可自身递归**重入的，在递归的过程中为了避免无限递归，如果发现相同的宏在之前的递归中已经展开过，则不再展开。
3. 参数之间使用逗号分隔（额外用括号包住的内容视为一个参数），参数的个数需要跟宏定义的个数一致，参数允许为空。

```cpp
额外用括号包住的内容视为一个参数额外用括号包住的内容视为一个参数额外用括号包住的内容视为一个参数#define AAA 666
#define BBB 888
#define AAABBB 2333
#define LINK_PARAMS(var1, var2) (var1 ## var2)
...
int main()
{
    // 先替换宏LINK_PARAMS，进行连接后返回结果 AAABBB，
    // 而AAABBB的宏定义为 2333，所以最后替换为宏定义AAABBB；
    cout << LINK_PARAMS(AAA, BBB) << endl; // 输出：2333

    ITER(1, 2)          // -> ITER(2, 1)，禁止递归展开

    CONCAT(Hello, World)                // -> HelloWorld
    CONCAT(Hello, CONCAT(World, !))     // ->　HelloCONCAT(World, !)，##组织后续的CONCAT(World, !)宏展开

ADD_COMMA(a, b)             // -> a, b
ADD_COMMA(a)                // 报错 "macro "MACRO" requires 2 arguments, but only 1 given"
ADD_COMMA((a, b), c)        // -> (a, b), c，额外用括号包住的内容视为一个参数
...
// 可变参数3：参数长度必须不小于1，否则在标准C宏展开后在末尾会多一个逗号，在GNU C++可以通过##__VA_ARGS__避免
#define eprintf(format, …) fprintf (stderr, format, __VA_ARGS__)


#define STRINGIZE(arg0) # arg0
STRINGIZE(a)                // -> "a"
STRINGIZE(STRINGIZE(a))     // -> "STRINGIZE(a)"，#字符串化，阻止继续展开

#define CONCAT(arg0, arg1) arg0 ## arg1
CONCAT(Hello, World)                        // -> HelloWorld
CONCAT(Hello, CONCAT(World, !))             // -> HelloCONCAT(World, !)
CONCAT(CONCAT(Hello, World) C, CONCAT(!))    // -> CONCAT(Hello, World) CCONCAT(!) ##拼接，阻止继续展开
...
| 符号                       | 动作与含义                                      |
|:------------------------:|:------------------------------------------:|
| #                        | 字符串化宏参数，忽略参数前后的空格，参数中间的多个空格转换为一个空格（组织继续展开） |
| ##                       | 符号连接（阻止##后面的宏继续展开）                         |
| #@                       | 字符化宏参数，当参数超过四个字符编译器报错                      |
| \                        | 续行（\后面**不能有空格**）                           |
| `...`与 `_\_VA\_ARGS\_\_` | 可变参数1：前部分表示可变参数，后半部分代表这部分参数                |
| `args...`与 `args`        | 可变参数2：前部分表示可变参数，后半部分代表这部分参数                |
...
| `__DATE__`         | 处理器上当前的日期                               |
| `__TIME__`         | 处理器上当前的时间                               |
| `__STDC_VERSION__` | C 标准的版本                                 |
| `__cplusplus__`    | 在 C++ 编译器时会被定义，展开结果同 `__STDC_VERSION__` |
| `__FUNCTION__`     | 包含类作用域的函数名                              |

## 编译器属性

### `__attribute__`

`__attribute__`是一个编译属性（GNU-C的扩展之一），用于向编译器描述特殊的标识、错误检查或高级优化，可以放在**函数，变量和类型声明之后**设置函数属性（Function Attribute ）、变量属性（Variable Attribute ）和类型属性（Type Attribute)等。其语法形式为 `__attribute__ ((attribute-list)) `， 后面跟两组括号（两个括号可以让它很容易在宏里面使用，特别是有多个属性的时候）。在括号里面是用逗号分隔的属性列表 `attribute-list`。

| attribute                                                               | 作用                                     | 作用对象  |
|:-----------------------------------------------------------------------:|:--------------------------------------:|:-----:|
| `format`                                                                | 检查格式化字符串                               | 函数    |
| `constructor`                                                           | （通过在ELF中添加 `.ctors`段）确保函数在 `main`之前被调用 | 函数    |
| `destructor`                                                            | （通过在ELF中添加 `.dtors`段）确保函数在 `main`之后被调用 | 函数    |
| [`cleanup`](http://blog.sunnyxx.com/2014/09/15/objc-attribute-cleanup/) | 在作用域结束时自动执行指定的方法                       | 变量    |
| `used`                                                                  | 通知编译器在链接时保留该符号（即使没有使用）                 | 变量、函数 |
| `noreturn`                                                              | 函数不需要返回值（可用于函数未执行到 `return`就需退出的情况）    | 函数    |
| `nonnull`                                                               | 指定函数的的某些参数不能是空指针                       | 函数参数  |
| `noinline & always_inline`                                              | 控制函数是否内联（语法层面的 `inline`只是对编译器的请求，不同于此） | 函数    |
| `warn_unused_result`                                                    | 要求调用者必须检查或者使用返回值                       | 函数    |
| `pure`                                                                  | 除了返回值以外没有任何副作用（返回值只依赖于传入的参数和/或全局变量）    | 函数    |
| `const`                                                                 | 除了返回值以外没有任何副作用（个函数除了参数之外不会对值进行检查）      | 函数    |

pure 和 const 都是为了支持高效的性能优化而营造出函数式编程范例的属性。const 可以被看做是更加严格的 pure，因为它不依赖于全局变量或者指针。被声明为 const 的函数结果除了传入参数之外不依赖于任何东西，所以这个函数的结果就可以被缓存起来，当之后用同样的参数调用的时候可以直接把缓存返回。

```cpp
/////////////// __attribute__((format())) \\\\\\\\\\\\\\\\\\\\\\\\
format (archetype, string-index, first-to-check)//format属性
/*
archetype: 指定是哪种风格，如printf，scanf
...
extern void* my_memcpy (void *dest, const void *src, size_t len) __attribute__((nonnull (1, 2)));//my_memcpy的前两个参数不能是空指针
```
...
#### [CLANG特有属性](https://www.jianshu.com/p/965f6f903114)

使用之前可以使用使用 `__has_attribute`指令检查编译器是否支持该属性

| attribute      | 作用                         | 作用对象 |
|:--------------:|:--------------------------:|:----:|
| `availability` | 在声明中描述跟系统版本有关的生命周期         | 函数   |
| `unavailable`  | 编译时告知方法不可用                 | 函数   |
| `overloadable` | 提供重载支持，调用时编译器会自动根据参数选择函数原型 | 函数   |
...
16. [面试系列之C++的对象布局【建议收藏】](https://segmentfault.com/a/1190000023098876)
17. [C++中的inline用法](https://segmentfault.com/a/1190000023098876)
18. [`__attribute__`](https://nshipster.cn/__attribute__/)
19. [C/C++ 宏编程解析](https://disenone.github.io/2021/03/31/cpp-preprocess)
20. [C/C++ 宏编程的艺术](https://zhuanlan.zhihu.com/p/152354031)
21. [C++ 模板参数推导 | 6cdh's Blog](https://6cdh.github.io/posts/template-argument-deduction/)
