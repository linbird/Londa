...
#### [类型推导中的类型转换](https://www.jianshu.com/p/589a747557ce)

只能用于推导模板函数形参，不能用于推导返回值。在类型推导的时候自动的类型转换是受限制的：
...
模板函数通过特化模板参数（**只支持全特化**），可以定义同名的模板函数，在模板实例化时，如果可以同时匹配全特化的模板函数和一般模板函数时，编译器会优先使用特化后的模板（特化后的模板的实现可以独立与普通模板），因此实现了**模板函数的重载**。
